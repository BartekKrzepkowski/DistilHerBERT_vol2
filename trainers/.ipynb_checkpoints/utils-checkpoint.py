...
def dynamically_unfreeze_layers(model, epoch_current, epoch_max, step):
    epoch -= 2
    if epoch >= 0:
        for name, params in model.bert.embeddings.named_parameters():
            if params.requires_grad:
...
def configure_optimizer(backbone, head, optim, lr_backbone, lr_head, weight_decay=1e-4, **optim_kwargs):
