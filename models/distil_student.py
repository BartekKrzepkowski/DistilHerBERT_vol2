...
def copy_weights_from_teacher_to_student(teacher, student):
    if isinstance(teacher, BertModel) or isinstance(teacher, BERT) or isinstance(teacher, BertForMaskedLM):
...
def create_student(teacher_model=None):
    if teacher_model is None:
        teacher_model = AutoModelForMaskedLM.from_pretrained("allegro/herbert-base-cased")
    else:
        import copy
